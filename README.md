...
- **沙箱里看不到 GPU / 缓存目录不可写**：可写工作区默认只能写项目目录和 /tmp，bubblewrap 的 `/dev` 也只有最基本的设备节点。
  Longx 会自动把 `/dev/nvidia*`、`/dev/dri` 放进沙箱（CUDA 可用），并在「项目设置 → 沙箱额外可写目录」里默认放行 `~/.cache`
  （uv / pip / npm 的缓存）；别的目录（比如数据集）加在同一处，一行一个。
