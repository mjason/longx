...
export function rpcMock() {
  return {
    ...
    gitChanges: vi.fn(async () => ok({ repository: false, branch: null, head: null, changes: [], ahead: null, behind: null, remotes: [], lfs: false })),
    gitFileDiff: vi.fn(async () => ok({ binary: false, diff: "" })),
    ...
    gitCommitFileDiff: vi.fn(async () => ok({ binary: false, diff: "" })),
