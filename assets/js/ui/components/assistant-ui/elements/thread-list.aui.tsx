...
const dateGroupLabel = (
  ...
  if (!date || date.getTime() >= startOfToday) return t.today;
  if (date.getTime() >= startOfToday - DAY_IN_MS) return t.yesterday;
  return t.earlier;
};
  ...
  return useMemo(() => {
    ...
    const filteredIndices = threadIds
      ...
      .filter(
        ({ id }) =>
          !query ||
          (itemsById.get(id)?.title || t.untitledThread)
            .toLowerCase()
            .includes(query),
      )
      .map(({ index }) => index);
