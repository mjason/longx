...
// Adapted from the catalog's single-select list: ours is a checklist (the
// models a template adds), `price` optional, a `note` where the price was
// (e.g. "已添加") and `disabled` rows that cannot be toggled.
...
export function ModelPicker({
  ...
  ComponentProps<"div">,
  "children" | "models" | "selectedIds" | "onToggle"
> & {
            ...
            .map((model) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex items-center gap-2.5 rounded-xl px-2 py-2 text-start transition-colors disabled:opacity-50",
                    selected
                      ? "bg-foreground/[0.06]"
