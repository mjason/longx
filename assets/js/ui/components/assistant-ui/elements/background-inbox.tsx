...
// Longx: `title` and `countLabel` replace the demo's English copy.
export function BackgroundInbox({
  runs,
  onCollect,
  title = "Running elsewhere",
  countLabel = (ready, running) => (ready > 0 ? `${ready} ready` : `${running} in flight`),
  className,
  ...props
}: Omit<ComponentProps<"div">, "children" | "runs" | "onCollect" | "title"> & {
  runs: readonly BackgroundRun[];
  onCollect?: (id: string) => void;
  title?: string;
  countLabel?: (ready: number, running: number) => string;
}) {
