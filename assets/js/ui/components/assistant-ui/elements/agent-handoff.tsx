...
// Longx: `carriedLabel` replaces the demo's "carried over"; `carried` is optional.
export function AgentHandoff({
  from,
  to,
  reason,
  carried = [],
  carriedLabel = "carried over",
  settled,
  className,
  ...props
}: Omit<
    ...
    >
      ...
      {carried.length > 0 && (
        <div className="flex flex-col gap-1">
          <span className={cn(mono, "text-foreground/30")}>{carriedLabel}</span>
          {carried.map((item) => (
            <span
              key={item}
