...
import { listTurns, redoTurn, restoreFiles, restoreProposal } from "@/ash_rpc";

const turns = [
  { id: "tu1", codexTurnId: "turn_1", userText: "add a test", modelSlug: "deepseek-flash", status: "completed", startedAt: "2026-09-13T01:00:00Z", completedAt: "2026-09-13T01:00:09Z", commitBefore: "aaaa1111bbbb", commitAfter: "cccc2222dddd", dirtyStart: false, diff: "diff --git a/lib/a.ex b/lib/a.ex\n--- a/lib/a.ex\n+++ b/lib/a.ex\n@@ -1 +1 @@\n-old\n+new\n", error: null },
  { id: "tu2", codexTurnId: "turn_2", userText: "and docs", modelSlug: "deepseek-flash", status: "failed", startedAt: "2026-09-13T01:05:00Z", completedAt: "2026-09-13T01:05:03Z", commitBefore: "cccc2222dddd", commitAfter: null, dirtyStart: false, diff: null, error: "codex restarted while this turn was running" },
];
...
describe("TurnsTool (history)", () => {
  ...
  test("lists the thread's turns with status, model and commits; the diff opens per file", async () => {
    ...
    expect(within(panel).getByText(/codex restarted/)).toBeInTheDocument();
  ...
  test("redo: text, model and mode go to redo_turn", async () => {
    const { user, panel } = await openTool();
    await user.click(within(panel).getAllByRole("button", { name: /重跑/ })[0]!);
    const dialog = await screen.findByRole("dialog");
    const text = within(dialog).getByRole("textbox");
    await user.clear(text);
    await user.type(text, "add a better test");
    await user.click(within(dialog).getByRole("button", { name: "重跑这一轮" }));
    await waitFor(() => expect(redoTurn).toHaveBeenCalledWith(expect.objectContaining({ input: expect.objectContaining({ turnId: "tu1", text: "add a better test", mode: "revert" }) })));
  });
});
