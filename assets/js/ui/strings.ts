...
export const t = {
  ...
  cachePreset: (label: string) => `添加 ${label} 缓存`,
  sandboxHint: {
    writable: (dir: string) => `沙箱不让写 ${dir}。允许这个项目写它？`,
    network: "沙箱关着联网与本机服务，这条命令要连本机 socket（GPU 驱动、Docker 之类）。打开？",
    gpu: "沙箱里看不到这台机器的 GPU。把 GPU 设备放进这个项目的沙箱？",
    launch: "沙箱本身起不来——看「设置 → 沙箱与权限」。",
    allow: "允许",
    allowed: "已允许，下一轮起生效",
    allowedRestart: "已允许，重启这个项目的 codex 后生效",
    settings: "在项目设置里可以改回来",
