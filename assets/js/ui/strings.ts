...
export const t = {
  ...
  passthroughHint: "一行一个，可用通配符。GPU、USB、串口这类设备和宿主的 socket 是沙箱看不到、agent 也申请不了的东西：命令因看不到 GPU 失败时聊天里会提示并一键放行；别的设备手动写在这里。每一项都在放宽隔离（Docker socket 等于宿主 root）。codex 启动时读取，改了要重启 codex（状态栏会提示）。仅 Linux 沙箱有效。",
  sandboxAdvanced: "长期放开的目录和设备（高级）",
  sandboxAdvancedHint: "平时不用碰：agent 缺什么权限会在聊天里申请，批准只对本轮或本会话有效。这里写的是对这个项目长期有效的例外。",
  sandboxHint: {
    gpu: "沙箱里看不到这台机器的 GPU（codex 的权限申请表达不了设备）。把 GPU 设备放进这个项目的沙箱？",
