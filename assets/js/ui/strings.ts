...
export const t = {
  ...
  agentKernel: {
    hint: "原生内核的团队参数：agent 能派多深、同时几个、闲多久自己退出，孩子默认跑哪个模型，reviewer 角色固定跑哪个模型。这些是每个 agent 描述最上面的一层，项目设置里可以覆盖。",
    maxDepth: "派出深度上限",
    ...
    saved: "已保存",
    files: "全局的 agent 文件",
    filesHint: "agent.exs、agents/<名字>/agent.exs + prompt.md、plugs/*.exs——对所有项目生效，放在数据目录里。",
    noFiles: "还没有全局文件。",
    newFile: "新建文件",
    newPath: "路径",
    newPathHint: "如 agents/writer/agent.exs、plugs/deploy.exs、agents/writer/prompt.md",
    templateAgent: (name: string) => `import Longx.Agent.Config\n\nagent do\n  version 1\n  summary "${name} 是干什么的，一句话"\n  prompt_file "prompt.md"\n  agents []\nend\n`,
    templatePlug: (name: string) => `defmodule ${name} do\n  use Longx.Agent.Plug\n\n  instructions "…"\nend\n`,
    remove: "删除",
