...
export const t = {
  ...
  sandbox: "沙箱",
  approval: "审批",
  network: "联网与本机服务（workspace-write 沙箱内）",
  networkHint: "关着时命令连不上任何东西，包括本机的服务和 socket：Docker、本地数据库、GPU 驱动初始化用的 socket 都算。",
  passthrough: "放进沙箱的宿主路径",
  passthroughHint: "一行一个，可用通配符。USB、串口、宿主 socket 这类设备是沙箱看不到、agent 也申请不了的东西，手动写在这里（GPU 用上面的开关）。每一项都在放宽隔离（Docker socket 等于宿主 root）。codex 启动时读取，改了要重启 codex（状态栏会提示）。仅 Linux 沙箱有效。",
  gpuPassthrough: "把 GPU 放进沙箱",
  gpuPassthroughHint: "这台机器有 GPU，沙箱默认看不到它（codex 的权限申请也表达不了设备）。打开后 codex 启动时把这台机器的 GPU 设备节点绑进沙箱——换机器不用改，改了要重启 codex。仅 Linux 沙箱有效。",
  ...
  sandboxHint: {
    ...
    cudaNetwork: "CUDA 初始化要连驱动的本机 socket，沙箱关着「联网与本机服务」时会被拦。打开？",
    allow: "允许",
    allowed: "已允许，下一轮起生效",
    allowedRestart: "已允许，重启这个项目的 codex 后生效",
