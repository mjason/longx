...
export const t = {
  ...
  filterFiles: "按文件名查找…",
  watchOff: "文件监控没有在运行：磁盘上的改动不会自己出现，点刷新更新。",
  watchPartial: "有的目录没能监控（多半是系统的 inotify 上限），那里的改动要点刷新。",
  ...
  fileRules: {
    hint: "项目页面开着的时候，Longx 监控项目里的文件：文件树、Git 窗口和 agent 描述随磁盘自己更新；页面都关了，监控也停。这里的规则决定哪些路径不看——被忽略的目录不监控、在文件树里变暗、@ 搜索也跳过。",
    layers: "规则按 gitignore 的写法，一层压一层，后面的说了算：内置 → 这里 → 项目设置 → .gitignore → 始终监控 → 项目根目录的 .longxignore（最高，!target/reports/ 可以把 .gitignore 忽略的目录要回来）。",
    ignore: "忽略",
    watch: "始终监控",
    placeholder: { ignore: "logs/\n*.tmp", watch: "generated/api/" },
    hints: {
      ignore: "每行一条，gitignore 写法；# 开头是注释。",
      watch: ".gitignore 忽略了、但仍要看的路径（例如 agent 的描述 .longx/）。",
    },
    builtin: "内置：",
    save: "保存规则",
    project: "文件监控规则",
    projectHint: "只对这个项目生效，压在全局规则之上。项目根目录的 .longxignore 比这里更高，跟着代码进 git。",
  },
  processes: {
