...
import { RpcFailure, useCreateProject, type NewProjectInput } from "@/core/projects";
    ...
    <>
      ...
      <Page>
        <form id="project-details" onSubmit={submit} className="flex max-w-lg flex-col gap-5" noValidate>
          ...
          <button type="button" className="text-muted-foreground touch-target self-start text-sm underline" onClick={() => setAdvanced((v) => !v)} aria-expanded={advanced}>
            {t.advanced}
          </button>
          {advanced ? (
            <div className="grid gap-4 rounded-lg border p-4">
              <div className="grid gap-2">
                ...
                <Select value={sandbox} onValueChange={(v) => setSandbox(v as typeof sandbox)}>
                  <SelectTrigger className="h-11 w-full"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {Object.entries(t.sandboxOptions).map(([k, label]) => <SelectItem key={k} value={k}>{label}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="grid gap-2">
                ...
                <Select value={approval} onValueChange={(v) => setApproval(v as typeof approval)}>
                  <SelectTrigger className="h-11 w-full"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {Object.entries(t.approvalOptions).map(([k, label]) => <SelectItem key={k} value={k}>{label}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
