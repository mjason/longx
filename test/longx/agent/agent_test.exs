defmodule Longx.AgentTest do
  ...
  test "a question, an answer: events, transcript, request", %{bypass: bypass, thread_id: id} do
    ...
    assert Enum.map(body["tools"], & &1["name"]) |> Enum.sort() ==
             ~w(apply_patch create_goal credential_create credential_login credentials_list exec_command get_context_remaining get_goal http_request knowledge_read knowledge_search knowledge_write new_context_window present prompt_user update_goal view_image web_fetch web_search)
