defmodule Longx.Codex.HomeTest do
  ...
  describe "the exec-server (codex's commands run through Longx)" do
    @gateway "http://127.0.0.1:4242/ai/v1"

    test "prepare/1 names it in environments.toml as the only environment; without a url codex keeps its own executor",
         %{dir: dir} do
      url = "ws://127.0.0.1:4242/exec/p1?token=t"
      {:ok, _} = Home.prepare(dir: dir, gateway_url: @gateway, exec_server_url: url)

      assert File.read!(Path.join(dir, "environments.toml")) == """
             # Generated by Longx (Longx.Codex.Home) on every boot — do not edit.
             # codex runs its commands and reads its files through Longx's exec-server.
             default = "longx"
             include_local = false
      ...
      # the token in the url is a secret: the file is the owner's alone
      assert File.stat!(Path.join(dir, "environments.toml")).mode |> Bitwise.band(0o077) == 0

      {:ok, _} = Home.prepare(dir: dir, gateway_url: @gateway)
      refute File.exists?(Path.join(dir, "environments.toml"))
      # the file is not part of what makes a home stale (the url carries a per-boot token)
      assert Home.stale(dir, gateway_url: @gateway) == []
    end

    test "exec_server_url/1 is the endpoint's loopback port, the project's id and the gateway token; nil where codex sandboxes itself (Windows)" do
      url = Home.exec_server_url("p1")
      port = LongxWeb.Endpoint.config(:http)[:port]
      assert url == "ws://127.0.0.1:#{port}/exec/p1?token=#{URI.encode_www_form(Token.current())}"
      assert Home.exec_server_url("p1", platform: {:windows, :x86_64}) == nil
      assert Home.exec_server_url("p1", platform: {:darwin, :aarch64}) == url
    end
  end
end
